...
public class CatPositionGenerator : MonoBehaviour
{
    ...
    //// 이전 고양이 위치 선택이 잘못돼서 백트래킹하는 경우, 피해야하는 고양이 위치
    ///// TODO) 최상위 Row에서 변경되는 경우 초기화 되어야함. << 방법 고민(현재 재귀 코드에서는 내가 최상위에서 변경되는지 알 수 없음)
    ...
    // 다음 row에서 반복(재귀)하되, 배치 불가능하면 이전 row의 배치된 고양이를 다른 위치로 변경
    // 모든 row에서 배치하면 종료
    void DecideCatInRow(int rowIndex)
    {
        ...
        else
        {
            ...
            if (TryGetCatPositionInRow(rowIndex, out Vector2Int lastRowCatPos))
            {
                ...
                // 이전 row 고양이 위치를 배치 불가능한 상태로 변경
                //failedCandidates.Add(lastRowCatPos);
            }
            else
            {
                Debug.LogError($"CatPositionGenerator Error :: 이전 Row의 고양이 찾기 실패");
                return;
            }
        }
